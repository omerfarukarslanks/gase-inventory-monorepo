...
export default function AuthCallbackCard() {
  ...
  useEffect(() => {
    if (!token) {
      setError("Geçersiz giriş bağlantısı.");
          ...
          <div className="animate-si text-center">
            <div className="mx-auto mb-6 flex h-[72px] w-[72px] items-center justify-center rounded-2xl bg-error/10">
              <svg
                ...
                viewBox="0 0 24 24"
                fill="none"
                stroke="rgb(var(--error, 239 68 68))"
                strokeWidth="1.5"
              >
            ...
            <h1 className="mb-2 text-[22px] font-bold tracking-tight text-text">Giriş Başarısız</h1>
            <p className="mb-4 text-[13.5px] leading-relaxed text-muted">{error}</p>
            <p className="text-[12.5px] text-muted">Giriş sayfasına yönlendiriliyorsunuz...</p>
          </div>
        </div>
      </div>
    );
