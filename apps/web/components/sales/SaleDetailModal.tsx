...
function formatAmountWithCurrency(value: number | string | null | undefined, currency?: string | null) {
...
function SaleDetailContent({
  ...
  error,
  detail,
}: Pick<SaleDetailModalProps, "loading" | "error" | "detail">) {
  if (loading) {
    return <p className="text-sm text-muted">Satis fis detayi yukleniyor...</p>;
  }
  ...
  if (!detail) {
    return <p className="text-sm text-muted">Detay bulunamadi.</p>;
  }
    ...
    <div className="space-y-4">
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="rounded-xl border border-border bg-surface2/30 p-3">
          ...
          <p className="text-xs text-text2">Odenen: {formatAmountWithCurrency(detail.paidAmount, detail.currency)}</p>
            ...
            {detail.lines.length === 0 ? (
              ...
              detail.lines.map((line) => (
                <tr key={line.id} className="border-b border-border text-sm text-text2">
                  ...
                  <td className="px-4 py-3 text-right font-medium text-text">{formatPrice(line.lineTotal)}</td>
                </tr>
              ))
            )}
          </tbody>
...
export default function SaleDetailModal({ open, loading, error, detail, onClose }: SaleDetailModalProps) {
  ...
  if (isMobile) {
    return (
      <Drawer
        ...
        onClose={onClose}
        side="right"
        title="Satis Fis Detayi"
        description={detail?.receiptNo ?? "-"}
        mobileFullscreen
      >
        <div className="space-y-4 p-5">
          <SaleDetailContent loading={loading} error={error} detail={detail} />
        </div>
      </Drawer>
    );
  }
