...
type SaleReturnsMobileListProps = {
...
export default function SaleReturnsMobileList({
  items,
  loading,
  error,
  onOpenDetail,
  footer,
}: SaleReturnsMobileListProps) {
  const { t } = useLang();
                ...
                <article key={item.id} className="space-y-4 rounded-xl2 border border-border bg-surface p-4">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-xs text-muted">
                      {(item.saleReference ?? item.saleId ?? "-")} · {buildCustomerName(item)}
                    </p>
                  </div>

                  <dl className="grid grid-cols-2 gap-3 text-sm text-text2">
                    <div>
                      <dt className="text-[11px] font-semibold uppercase tracking-wide text-muted">{t("salesReturns.storeLabel")}</dt>
                      <dd className="mt-1">{item.store?.name ?? "-"}</dd>
                    </div>
                    <div>
                      <dt className="text-[11px] font-semibold uppercase tracking-wide text-muted">{t("salesReturns.returnedAtLabel")}</dt>
                      <dd className="mt-1">{formatDate(item.returnedAt ?? undefined)}</dd>
                    </div>
                    <div>
                      <dt className="text-[11px] font-semibold uppercase tracking-wide text-muted">{t("salesReturns.lineCountLabel")}</dt>
                      <dd className="mt-1">{item.lineCount}</dd>
                    </div>
                    <div>
                      <dt className="text-[11px] font-semibold uppercase tracking-wide text-muted">{t("salesReturns.totalRefundLabel")}</dt>
                      <dd className="mt-1 font-medium text-text">{formatPrice(item.totalRefundAmount)}</dd>
                    </div>
                    {item.notes ? (
                      <div className="col-span-2">
                        <dt className="text-[11px] font-semibold uppercase tracking-wide text-muted">{t("salesReturns.notesLabel")}</dt>
                        <dd className="mt-1 text-text2">{item.notes}</dd>
                      </div>
                    ) : null}
                  </dl>
