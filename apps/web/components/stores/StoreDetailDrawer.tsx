...
export default function StoreDetailDrawer({ store, onClose }: Props) {
  return (
    <Drawer open={Boolean(store)} onClose={onClose} side="bottom" mobileFullscreen>
      {store ? <StoreDetailContent store={store} trailingAction={<CloseButton onClose={onClose} />} /> : null}
    </Drawer>
  );
}
