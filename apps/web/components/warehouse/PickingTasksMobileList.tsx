...
import type { PickingTaskListItem } from "@/components/warehouse/PickingTasksTable";
import { useLang } from "@/context/LangContext";

type PickingTasksMobileListProps = {
  loading: boolean;
  error: string;
...
export default function PickingTasksMobileList({
  loading,
  error,
  items,
  onOpenDetail,
  footer,
}: PickingTasksMobileListProps) {
  const { t } = useLang();
              ...
              items.map((item) => (
                <article key={item.id} className="space-y-4 rounded-xl2 border border-border bg-surface p-4">
                  ...
                  <dl className="grid grid-cols-2 gap-3 text-sm text-text2">
                    <div>
                      <dt className="text-[11px] font-semibold uppercase tracking-wide text-muted">{t("warehouse.pickingTasks.wave")}</dt>
                      <dd className="mt-1">{item.waveCode}</dd>
                    </div>
                    <div>
                    ...
                    <div className="col-span-2">
                      <dt className="text-[11px] font-semibold uppercase tracking-wide text-muted">{t("warehouse.pickingTasks.sourceLocation")}</dt>
                      <dd className="mt-1">{item.sourceLabel}</dd>
                    </div>
                  </dl>
