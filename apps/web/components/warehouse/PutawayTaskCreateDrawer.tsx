...
import { getWarehouseLocations, type CreatePutawayTaskPayload, type WarehouseLocation } from "@/lib/warehouse";

type PutawayTaskCreateDrawerProps = {
  open: boolean;
  submitting: boolean;
  warehouseOptions: Array<{ value: string; label: string }>;
  initialWarehouseId?: string;
  onClose: () => void;
  onSubmit: (payload: CreatePutawayTaskPayload) => Promise<void>;
};
...
export default function PutawayTaskCreateDrawer({
  open,
  submitting,
  warehouseOptions,
  initialWarehouseId,
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        const locations = await getWarehouseLocations(resolvedWarehouseId);
        if (!cancelled) {
          setLocationOptions(locations);
          if (!locations.some((location) => location.id === toLocationId)) {
            setToLocationId("");
          }
        }
      } catch {
  ...
  }, [resolvedWarehouseId, toLocationId]);

  const handleSubmit = async () => {
    if (!resolvedWarehouseId) {
      setFormError(t("warehouse.putawayTasks.warehouseRequired"));
      return;
    }
    if (!selectedVariantId) {
      setFormError(t("warehouse.putawayTasks.variantRequired"));
      return;
    }
    if (!toLocationId) {
      setFormError(t("warehouse.putawayTasks.locationRequired"));
      return;
    }
    const parsedQuantity = Number(quantity);
    if (Number.isNaN(parsedQuantity) || parsedQuantity <= 0) {
      setFormError(t("warehouse.putawayTasks.quantityRequired"));
      return;
    }
  ...
  return (
    <Drawer
      ...
      onClose={onClose}
      side="right"
      title={t("warehouse.putawayTasks.createTitle")}
    ...
    >
      <div className="space-y-4 p-5">
        ...
        <div>
          <label className="mb-1 block text-xs font-semibold text-muted">{t("warehouse.putawayTasks.destinationLocation")} *</label>
          <SearchableDropdown
        ...
        <div>
          <label className="mb-1 block text-xs font-semibold text-muted">{t("warehouse.putawayTasks.quantityLabel")} *</label>
          <input
            type="number"
            min="0"
            step="1"
        ...
        <div>
          ...
          <textarea
            ...
            placeholder={t("warehouse.putawayTasks.notesPlaceholder")}
          />
        </div>
