...
export async function acceptReplenishmentSuggestion(id: string): Promise<ReplenishmentSuggestion> {
  return apiFetch<ReplenishmentSuggestion>(`/replenishment/suggestions/${id}/accept`, { method: "POST" });
}

export async function dismissReplenishmentSuggestion(
  id: string,
  notes: string,
...
export async function deactivateReplenishmentRule(id: string): Promise<void> {
  await apiFetch<unknown>(`/replenishment/rules/${id}`, { method: "DELETE" });
}
